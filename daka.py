...
def sign_in(id, pwd, name="Turing", check_today=1):
    ...
    proxy = [
        {
            'http': 'http://220.169.232.165:9999',
            'https': 'http://220.169.232.165:9999',
        },
        {
            'http': 'http://120.41.134.161:8888',
            'https': 'http://120.41.134.161:8888',
        },
        {
            'http': 'http://182.105.201.172:9999',
            'https': 'http://182.105.201.172:9999',
        },
        {
            'http': 'http://117.70.41.218:8080',
            'https': 'http://117.70.41.218:8080',
        },
        {
            'http': 'http://114.233.219.27:8080',
            'https': 'http://114.233.219.27:8080',
        },
    ]
    ...
    while True:
        try:
            ...
            r = s.post("https://jksb.v.zzu.edu.cn/vls6sss/zzujksb.dll/login", headers=headers, data=form,
                              proxies=random.choice(proxy), timeout=(200, 200), verify=False)  # response为账号密码对应的ptopid和sid信息,timeout=60(sec)
    ...
    while True:
        try:
            r = s.get(
                ...
                proxies=random.choice(proxy), headers=headers, verify=False)  # response里含有jksb对应的params
        except:
    ...
    while True:
        try:
            r = s.get(
                "https://jksb.v.zzu.edu.cn/vls6sss/zzujksb.dll/jksb?ptopid=" + ptopid + "&sid=" + sid + "&fun2=",
                proxies=random.choice(proxy), headers=headers, verify=False)  # response为jksb表单第一页
        except:
    ...
    while True:
        try:
            r = s.post("https://jksb.v.zzu.edu.cn/vls6sss/zzujksb.dll/jksb", headers=headers,
                              proxies=random.choice(proxy), data=form, verify=False)  # response为打卡的第二个表单
        except:
            while curr_punch > max_punch:
                exit()
            curr_punch += 1
    ...
    # 将图片验证码转换为文本列表
    vcode_list = utils.baidu_ORC(dest)
    # 手写api为”“，高精度不为”“ 返回高精度
    if vcode_list[0] == "" and vcode_list[1] != "":
        vcode_text = vcode_list[1]
    # 手写api为”“，高精度为”“ 返回常规
    elif vcode_list[1] == "" and vcode_list[0] == "":
        vcode_text = vcode_list[2]
    # 其余情况返回 手写识别
    else:
        vcode_text = vcode_list[0]
    # 如果是大写中文数字再转换为阿拉伯数字
    ...
    while True:
        try:
            r = s.post("https://jksb.v.zzu.edu.cn/vls6sss/zzujksb.dll/jksb", data=form,
                              proxies=random.choice(proxy), headers=headers, verify=False)  # response为完成打卡页面
        except:
            while curr_punch > max_punch:
                exit()
            curr_punch += 1
